...
* 그리고, 기존에 datasource 코드에서 민감한 정보들이 github repository에 그대로 노출되는 코드들이 있었는데, 이 부분에 대해서는 노출하지 않는 것이 무조건 맞다고 생각한다.
  * 민감한 정보를 노출하지 않는 방법으로는 외부 파일로 뺀다던가, 아니면 config 서버 같은 것을 따로 구축하는 방법 등이 있을 것 같다.
  * 그런데 현재 github action을 사용하고 있으니까, github action에서 직접 민감한 환경세팅 값을 미리 세팅해 두고 build 시 이용하는 방법으로 수정했다.
    * 이렇게 하면, 만약 추후에 DB 관련 세팅값 들이 변한다고 하더라도 내 코드는 전혀 건드리지 않아도 된다는 장점이 있다.
